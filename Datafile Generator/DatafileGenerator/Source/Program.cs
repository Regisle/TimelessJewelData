...
public static class Program
{
    ...
    public static void Main(string[] arguments)
    {
        ...
        var justNotables = Notables_List.Select(DataManager.GetPassiveSkillByFuzzyValue).Select(x => (int)x.GraphIdentifier).ToList();
        var justSmallNodes = nodeArray.Except(justNotables).ToList();
        justNotables.Sort();
        justSmallNodes.Sort();
        ...
        {
            var node = DataManager.GetPassiveSkillById(notablesThenSmalls[k]);
            sb.AppendLine(node.GraphIdentifier + "," + (node.Name.Contains(',') ? ("\"" + node.Name + "\"") : node.Name) + "," + k);
        }
        File.WriteAllText(Path.Combine(outputDir, "Node Indices.csv"), sb.ToString());
        ...
        for (int i = 1; i <= 5; i++)
        {
            ...
            if (i == 1)
            {
                ...
                Parallel.ForEach(nodeArray, nodeId =>
                {
                    ...
                    Parallel.For(jewel_min / jewel_increment, jewel_max / jewel_increment + 1, i =>
                    {
                        ...
                        header[notablesThenSmalls.IndexOf(nodeId) * maxSeed + jewel_index] = (byte)dataEntry.Count;
                        data[notablesThenSmalls.IndexOf(nodeId) * maxSeed + jewel_index] = dataEntry.ToArray();
                    });
                });
            ...
            else
            {
                ...
                Parallel.ForEach(justNotables, notableId =>
                {
                    var notable = DataManager.GetPassiveSkillById(notableId);
                    Parallel.For(jewel_min / jewel_increment, jewel_max / jewel_increment + 1, i =>
                    {
                        //which jewel seed is this
                        i *= jewel_increment;
                        ...
                        data[justNotables.IndexOf(notableId) * maxSeed + jewel_index] = passiveSkillIndex;
                    });
                });
                //wipe the file if it exists
                if (File.Exists(output_file))
