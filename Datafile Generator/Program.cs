...
#nullable disable
        string[] arguments)
        {
            ...
            var NotableIds = Notables_List.Select(Program.GetPassiveSkillFromInput);
            ...
            for (int i = 1; i <= 5; i++)
            {
                ...
                if (i == 1)
                {
                    Stopwatch gvsw = Stopwatch.StartNew();
                    //build the csv
                    ...
                    {
                        var node = GetPassiveSkillById(sortedNodes[k]);
                    ...
                    foreach (var nodeId in nodeArray)
                    {
                        var node = GetPassiveSkillById(nodeId);
                        Parallel.For(jewel_min / jewel_increment, jewel_max / jewel_increment + 1, i =>
                        {
                            ...
                            TimelessJewel timelessJewelFromInput = Program.GetTimelessJewelFromInput(jewel_seed, jewel_type);
                    ...
                    gvsw.Stop();
                    Console.WriteLine($"glorious vanity took {gvsw.Elapsed.TotalSeconds} seconds");
                    continue;
                }
                ...
                if (!indexed)
                {
                    ...
                    var sortedNotables = NotableIds.ToList();
                    ...
                    //make sure to change Notable_In.Index in the "bag.Add(new Tuple<uint, int, byte>(Notable_In.Index, i, passiveSkillIndex));" line too if you do though
                ...
                ConcurrentBag<Tuple<uint, int, byte>> bag = new ConcurrentBag<Tuple<uint, int, byte>>();
                Parallel.For(jewel_min / jewel_increment, jewel_max / jewel_increment + 1, i =>
                {
                    ...
                    if (timelessJewelFromInput == null)
                        Program.ExitWithError("Failed to get the [yellow]timeless jewel[/] from input.");
                    foreach (PassiveSkill Notable_In in NotableIds)
                    {
                        ...
                        //this is a piss poor way of doing it since itll need to be sorted later
                        //but this hopefully is only ever run a few times so I never fixed it
                        bag.Add(new Tuple<uint, int, byte>(Notable_In.Index, i, passiveSkillIndex));
                    }
                });
                //sort the output (lexicographic is default, which is what we want anyway)
                var list = bag.ToList();
                list.Sort();
                //write to the file
                using (Stream file = File.OpenWrite(output_file))
                {
                    file.Write(list.Select(x => x.Item3).ToArray(), 0, list.Count);
                }
                sw.Stop();
        ...
        public static PassiveSkill GetPassiveSkillById(int idval)
        {
            return DataManager.PassiveSkills == null ? (PassiveSkill)null : DataManager.PassiveSkills.FirstOrDefault<PassiveSkill>((Func<PassiveSkill, bool>)(q => q.GraphIdentifier == idval));
        }
        private static PassiveSkill GetPassiveSkillFromInput(string input)
        {

            PassiveSkill skillByFuzzyValue = DataManager.GetPassiveSkillByFuzzyValue(input);

            return skillByFuzzyValue;
        }

        private static TimelessJewel GetTimelessJewelFromInput(int input_seed, int input_type)
        {
            Dictionary<uint, string> source = new Dictionary<uint, string>()
      {
        {
          1U,
          "Glorious Vanity"
        ...
        },
        {
          ...
          "Elegant Hubris"
            ...
            Dictionary<uint, Dictionary<string, TimelessJewelConquerer>> dictionary = new Dictionary<uint, Dictionary<string, TimelessJewelConquerer>>()
      {
        {
          1U,
          new Dictionary<string, TimelessJewelConquerer>()
          {
            {
              "Xibaqua",
              new TimelessJewelConquerer(1U, 0U)
            },
            {
              "[yellow]Zerphi (Legacy)[/]",
              new TimelessJewelConquerer(2U, 0U)
            },
            {
              "Ahuana",
              new TimelessJewelConquerer(2U, 1U)
            },
            {
              "Doryani",
              new TimelessJewelConquerer(3U, 0U)
            }
          }
        },
        {
          ...
          new Dictionary<string, TimelessJewelConquerer>()
          {
            ...
            {
              "[yellow]Kiloava (Legacy)[/]",
              new TimelessJewelConquerer(3U, 0U)
            },
            {
              "Akoya",
              new TimelessJewelConquerer(3U, 1U)
            }
          }
        },
        {
          ...
          {
            {
              "[yellow]Deshret (Legacy)[/]",
              new TimelessJewelConquerer(1U, 0U)
            },
            {
              "Balbala",
              new TimelessJewelConquerer(1U, 1U)
            },
            {
              "Asenath",
              new TimelessJewelConquerer(2U, 0U)
        ...
        },
        {
          ...
          new Dictionary<string, TimelessJewelConquerer>()
          {
            ...
            Dictionary<uint, (uint, uint)> timelessJewelSeedRanges = new Dictionary<uint, (uint, uint)>()
      {
        {
          1U,
          (100U, 8000U)
        ...
        },
        {
          ...
          (2000U, 160000U)
            ...
            //string timelessJewelTypeInput = AnsiConsole.Prompt<string>((IPrompt<string>)new SelectionPrompt<string>().Title<string>("[green]Timeless Jewel Type[/]:").AddChoices<string>(source.Values.ToArray<string>()));
            //uint alternateTreeVersionIndex = source.First<KeyValuePair<uint, string>>((Func<KeyValuePair<uint, string>, bool>)(q => q.Value == timelessJewelTypeInput)).Key;
            //AlternateTreeVersion alternateTreeVersion = DataManager.AlternateTreeVersions.First<AlternateTreeVersion>((Func<AlternateTreeVersion, bool>)(q => (int)q.Index == (int)alternateTreeVersionIndex));
            AlternateTreeVersion alternateTreeVersion = DataManager.AlternateTreeVersions.ToList()[input_type];
            return new TimelessJewel(alternateTreeVersion, new TimelessJewelConquerer(1U, 0U), (uint)input_seed);
        }

        private static String PrintAlternatePassiveAdditionInformations(
          IReadOnlyCollection<AlternatePassiveAdditionInformation> alternatePassiveAdditionInformations)
        {
            ...
            foreach (AlternatePassiveAdditionInformation additionInformation in (IEnumerable<AlternatePassiveAdditionInformation>)alternatePassiveAdditionInformations)
            {
                //AnsiConsole.MarkupLine("\t[green]Addition[/]: " + additionInformation.AlternatePassiveAddition.Identifier);
                for (int index = 0; index < additionInformation.AlternatePassiveAddition.StatIndices.Count; ++index)
                {
                    /*uint num1 = additionInformation.AlternatePassiveAddition.StatIndices.ElementAt<uint>(index);
                    uint num2 = additionInformation.StatRolls.ElementAt<KeyValuePair<uint, uint>>(index).Value;
                    DefaultInterpolatedStringHandler interpolatedStringHandler = new DefaultInterpolatedStringHandler(25, 3);
                    interpolatedStringHandler.AppendLiteral("\t\tStat ");
                    ...
                    interpolatedStringHandler.AppendLiteral(", Roll: ");
                    interpolatedStringHandler.AppendFormatted<uint>(num2);
                    AnsiConsole.MarkupLine(interpolatedStringHandler.ToStringAndClear());*/
                    //Console.WriteLine(additionInformation.AlternatePassiveAddition.Identifier);
                    output = additionInformation.AlternatePassiveAddition.Identifier;
                    //Debug.WriteLine(additionInformation.AlternatePassiveAddition.Identifier);
                }
            }
            return output;
        }
        ...
        private static void PrintError(string error) => AnsiConsole.MarkupLine("[red]Error[/]: " + error);
