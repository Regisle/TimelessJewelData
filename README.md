...
Note that Node_Indices.csv contains indices for all modifiable nodes, but the non-Glorious Vanity jewels will only have data for notables (the lowest 391 indices). Modifications to non-notables for these jewels are consistent and thus have been omitted from their data files to save space.
...
Non-Glorious Vanity jewels are relatively simply and easy to parse with this definition:
